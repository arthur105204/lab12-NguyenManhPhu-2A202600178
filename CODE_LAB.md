...
|---------|-------|----------|---------------------|
| Config | Hardcode values trong code | Env vars qua `settings` | Tách cấu hình khỏi code, dễ deploy nhiều môi trường và tránh lộ secret |
| Health check | Không có | Có `/health` và `/ready` | Platform biết app còn sống và có sẵn sàng nhận traffic không |
| Logging | `print()` | Structured JSON logging | Dễ parse, dễ tìm lỗi, tích hợp hệ thống quan sát tốt hơn |
| Shutdown | Đột ngột | Graceful qua lifespan/SIGTERM | Tránh mất request đang xử lý và tắt container an toàn |
...
**Trả lời ngắn:**
1. Base image là image nền để build container; trong basic là `python:3.11`.
2. Working directory là thư mục làm việc mặc định trong container; ở đây là `/app`.
3. COPY `requirements.txt` trước để tận dụng Docker layer cache, tránh cài lại dependencies khi code đổi mà requirements không đổi.
4. `CMD` là lệnh mặc định có thể override khi chạy container; `ENTRYPOINT` là lệnh cố định hơn, thường dùng khi muốn container luôn chạy một executable nhất định.
...
**Trả lời ngắn:**
- Stage 1 (builder) cài build dependencies và Python packages cần để build.
- Stage 2 (runtime) chỉ copy những gì cần để chạy app.
- Image nhỏ hơn vì không mang theo tool build, cache, và file thừa từ stage build.
...
**Trả lời ngắn:**
- Services được start: `agent`, `redis`, `nginx`.
- `nginx` nhận traffic từ client và reverse proxy vào `agent`.
- `agent` dùng `redis` cho state/cache nếu cần.
- Các service giao tiếp qua Docker network nội bộ.

Test:

**Trả lời ngắn:**
- `railway.toml` là cấu hình riêng cho Railway.
- `render.yaml` là Blueprint config riêng cho Render.
- Cùng mục tiêu deploy app nhưng cú pháp, field và workflow khác nhau theo từng platform.

**Trả lời ngắn:**
- API key được check trong dependency/middleware trước khi vào endpoint `/ask`.
- Sai key thì trả `401 Unauthorized`.
- Rotate key bằng cách đổi secret trong environment variables và cập nhật client dùng key mới.

**Trả lời ngắn:**
- Dùng sliding window (thường lưu timestamp request trong Redis).
- Limit là 10 requests/minute.
- Admin có thể được exempt hoặc dùng bucket riêng với limit cao hơn trong logic auth/rate limiter.

**Giải thích ngắn:**
- Mỗi user có budget $10/tháng.
- Spending được track theo key `budget:{user_id}:{YYYY-MM}` trong Redis.
- Nếu tổng chi tiêu vượt 10 thì chặn request.

**Trả lời ngắn:**
- `/health` là liveness probe: kiểm tra process còn sống không.
- `/ready` là readiness probe: kiểm tra app đã sẵn sàng nhận traffic chưa.
- Nếu phụ thuộc chưa sẵn sàng thì nên trả `503`.

**Trả lời ngắn:**
- Khi nhận `SIGTERM`, app nên dừng nhận request mới, cho request đang chạy hoàn thành, đóng connection rồi thoát.

**Trả lời ngắn:**
- State không nên lưu trong memory của từng instance.
- Dùng Redis hoặc database chung để history/limit/budget dùng được across instances.

**Trả lời ngắn:**
- Nginx đóng vai trò load balancer.
- Khi scale `agent=3`, Nginx sẽ phân phối request sang các instance phía sau.
- Nếu một instance lỗi, traffic vẫn có thể đi sang instance còn lại.

**Trả lời ngắn:**
- Nếu conversation được lưu trong Redis thì khi instance bị kill, conversation vẫn còn.
- Đây là bằng chứng của stateless design.
```bash
# Health check
